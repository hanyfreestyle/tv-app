...
@section('content')
    ...
    <x-html-section>
        ...
        <div class="row mt-3">
            @if(count($Category->category_with_product_website)>0)
                <div class="row col-lg-12 hanySort">
                    @foreach($Category->category_with_product_website as $row)
                        <div class="col-lg-12"  data-index="{{$row->id}}" data-position="{{$row->postion_web}}" >
                            <p class="ListItem-12">{{$row->postion_web}} {{$row->name}} </p>
                        </div>
                    @endforeach
                </div>
            @else
...
@push('JsCode')
    ...
    <x-sort-ajax-code url="{{ route($PrefixRoute.'.ProductSaveSort') }}"/>
@endpush
